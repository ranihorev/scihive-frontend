...
// Based on https://github.com/react-spring/zustand/blob/master/src/middleware.ts
...
const devtools = (fn: any, prefix?: string) => (set: any, get: any, api: any) => {
  ...
  const namedSet = (state: any, name?: any) => {
    set(state);
    if (name) {
      const newState = pickBy(get(), value => {
        return typeof value !== 'function';
      });
      api.devtools.send(api.devtools.prefix + name, omit(newState, 'document'));
    }
  };
